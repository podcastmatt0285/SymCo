...
Energy ETF Bank - Commodity-backed investment fund for the power grid

Features:
- Asset valuation based on total market value of energy divided by total money supply
- Automated market making: buys at low prices, sells at high inventory levels
- IPO: Limited shares representing fractional ownership of energy reserves
...
BUY_PRICE_THRESHOLD = 0.80  # Buy when price drops to 80% of moving average
BUY_PREMIUM = 1.02  # Buy at 2% above current market price
BUY_PERCENTAGE = 0.05  # Buy 5% of total market supply

SELL_INVENTORY_THRESHOLD = 0.33  # Sell when we hold 33% of total supply
SELL_DISCOUNT = 0.95  # Sell at 5% below market price
...
# Dividend system
DIVIDEND_INTERVAL_TICKS = 12  # 7 days (weekly)
...
QE_BUY_INTERVAL = 3600
QE_BUY_PERCENTAGE = 0.10
...
class BankLien(Base):
    ...
    __tablename__ = "energy_etf_bank_liens"
...
def get_total_commodity_supply() -> float:
    """Calculate total energy in the entire game."""
    try:
        import inventory
...
def calculate_ipo_price() -> float:
    ...
    IPO Price = (Total Market Value of Energy) / (Total Money Supply)
    """
    try:
        import market
        
        total_energy = get_total_commodity_supply()
        ...
        if not market_price:
            ...
            if order_book['asks']:
                ...
                market_price = 0.10  # Energy is cheaper than seeds
                print(f"[{BANK_NAME}] No market data - using fallback price: ${market_price:.2f}")
        
        total_market_value = total_energy * market_price
        ...
        if ipo_price < 0.0001:
            print(f"[{BANK_NAME}] ⚠️ IPO price too low ({ipo_price:.8f}), using minimum $0.0001")
            ipo_price = 0.0001
        elif ipo_price > 1.0:
    ...
    try:
        ...
        if SHARE_ITEM_TYPE not in inventory.ITEM_RECIPES:
            inventory.ITEM_RECIPES[SHARE_ITEM_TYPE] = {
                "name": "Energy ETF Shares",
                "description": "Tradeable ETF shares backed by energy reserves",
                "category": "financial"
            }
...
def initialize():
    """Initialize the Energy ETF Bank."""
    import banks
...
def calculate_commodity_value() -> float:
    ...
    try:
        ...
        energy_held = inventory.get_item_quantity(BANK_PLAYER_ID, TARGET_COMMODITY)
        market_price = market.get_market_price(TARGET_COMMODITY)
        if not market_price:
            market_price = 0.10
        
        return energy_held * market_price
    except:
        return 0.0
    ...
    try:
        ...
        if current_price < (moving_avg * BUY_PRICE_THRESHOLD):
            ...
            if bank_entity.cash_reserves >= buy_cost and buy_quantity >= 1:
                ...
                if order:
                    ...
                    print(f"[{BANK_NAME}] 📉 BUY ORDER: {int(buy_quantity)} {TARGET_COMMODITY} @ ${buy_price:.4f}")
        ...
        if inventory_percentage >= SELL_INVENTORY_THRESHOLD and bank_inventory >= 1:
            ...
            if sell_quantity >= 1:
                ...
                if order:
                    print(f"[{BANK_NAME}] 📈 SELL ORDER: {int(sell_quantity)} {TARGET_COMMODITY} @ ${sell_price:.4f}")
...
def retire_bought_shares():
    ...
    try:
        ...
        print(f"[{BANK_NAME}] Share retirement: {bank_shares:.0f} total, {locked_shares:.0f} locked, {shares_to_retire:.0f} to retire")
...
def check_and_trigger_qe(current_tick: int):
    ...
    try:
        ...
        if order:
            ...
            print(f"[{BANK_NAME}] 🚨 QE BUY: {int(buy_quantity)} energy @ ${buy_price:.4f}")
...
async def tick(current_tick: int, now: datetime, bank_entity):
    ...
    if current_tick % 3600 == 0:
        try:
            ...
            energy_held = inventory.get_item_quantity(BANK_PLAYER_ID, TARGET_COMMODITY)
            total_supply = get_total_commodity_supply()
            inventory_pct = (energy_held / total_supply * 100) if total_supply > 0 else 0
            ...
            print(f"[{BANK_NAME}] {status}{qe_status} | NAV: ${nav:,.2f} | " +
                  ...
                  f"Energy: {energy_held:,.0f} ({inventory_pct:.1f}%) | " +
