...
class Player(Base):
    ...
    notif_push_crypto       = Column(Boolean, default=True)  # Push: crypto alerts (mining payouts, petitions, token launches)
...
def migrate_player_table():
    ...
    run_ddl_migration(engine, [
        ...
        "ALTER TABLE players ADD COLUMN IF NOT EXISTS notif_push_crypto BOOLEAN DEFAULT TRUE",
            ...
            <div class="hero-inner">
                ...
                <div class="faq-section">
                    ...
                    <details class="faq-item">
                        ...
                        <div class="faq-answer">The <strong>Port Authority</strong> is a sovereign military <strong>Institution</strong> — the same tier as the Mint. Like any institution it's built by <strong>sacrificing land</strong> (Wadsworth Pro required): create a Port Authority institution from Land → Institutions, then build your command on it. Once built, you move weapons and platforms from your inventory into <strong>Port Authority command</strong>, forming four <strong>Branches</strong> — Navy (carriers, subs, destroyers), Army (tanks, armored vehicles, rifles), Air Force (fighter jets, helicopters) and Intelligence (drones). Each unit type has an attack and defense value. Battles play out one unit at a time using a d20 dice system: the strongest units duel, the loser's unit is weakened (its stat erodes) until destroyed and permanently deleted. During admin-run <strong>Procurement Events</strong> you pick 3 targets and fight one battle per week, looting a slice of each defeated defender's inventory proportional to your surviving attack power — but if your whole force is wiped before you make it home, your loot is returned and you pay a steep trophy penalty (and a 60-day ban if you can't cover it). You can also <strong>blockade</strong> a rival anytime (freezing all their retail, market and district trading for up to 72h unless they defeat your forces), or spend Intelligence units to <strong>go dark</strong> and hide from a rival's target search. Units in command carry a daily upkeep cost, and the institution pays a monthly federal tax.<br><br>
                        <strong>Government Contracts:</strong> The admin periodically posts <strong>PA Contracts</strong> — government procurement tenders requiring specific weapons or platforms. Submit a bid with a refundable security deposit; if you win, fulfill the contract by shipping items from your regular inventory progressively (no need to have everything at once — ship what you have, and the progress bar fills over time). Completed contracts pay out <strong>tax-free</strong> in USD plus trophy rewards. A <strong>Chief Defense Officer (CDO)</strong> executive reduces your maintenance costs and boosts contract payouts.<br><br>
                        <strong>Immigration Policy:</strong> Use 10 tradeoff sliders to shape your immigration demographic — balancing advantages like many vs. wealthy arrivals, skilled vs. unskilled workers, urban vs. rural settlement, and more. Sliders feed into a 5-axis radar score (Retail Demand, Customer Loyalty, Production Output, Input Efficiency, Land Yield), each shifting up to ±4.6%. Commit your policy and it stays active for <strong>3 days</strong>, decaying back to neutral throughout. After expiry there's a <strong>7-day cooldown</strong> before you can commit again. Only affects your own businesses — not global.<br><br>
                        Manage yours at the <a href="/port-authority"><strong>Port Authority</strong></a> page.</div>
                    </details>
...
<div id="nav-loader" style="display:none;position:fixed;inset:0;z-index:9999;background:#0D0806;color:#F5F5DC;font-family:Georgia,serif;align-items:center;justify-content:center;padding:12px;">
  <div style="position:relative;width:100%;max-width:420px;padding:clamp(16px,5vw,40px);background:#1A0F0A;border:4px solid #2D1810;box-shadow:0 25px 50px rgba(0,0,0,.8);display:flex;flex-direction:column;align-items:center;box-sizing:border-box;max-height:92vh;overflow-y:auto;">
    <div style="width:min(90px,22vw);height:min(90px,22vw);flex-shrink:0;">
      <img src="/static/logo.png?v=3" alt="" style="width:100%;height:100%;object-fit:contain;filter:drop-shadow(0 0 12px rgba(176,141,87,0.5));">
    </div>
    <p style="font-size:13px;font-style:italic;color:#B08D57;margin:12px 0 6px;text-align:center;">Loading&hellip;</p>
    <div style="width:100%;height:3px;background:#2D1810;border-radius:2px;overflow:hidden;margin-top:4px;">
      <div id="nl-bar-page" style="height:100%;width:0%;background:linear-gradient(90deg,#B08D57,#e5c88a);transition:width .15s linear;border-radius:2px;"></div>
    </div>
  </div>
</div>
<script>
