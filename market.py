    ...
    if is_bank_ipo:
        ...
        if not petrodollar_handled:
            ...
            try:
                ...
                # Track all market sales taxes generated by this sell for Tax Contributor event.
                # Uses the 2.02% federal-tax equivalent unconditionally (applies even when buyer
                # is an NPC and no actual federal tax is charged), plus any city sales tax.
                if sell_order.player_id > 0:
                    _tax_event_total = _federal_tax + _tax_deducted
                    if _tax_event_total > 0:
                        try:
                            from events import record_task_progress as _rtp
                            _rtp(sell_order.player_id, "market_sales_tax_usd", _tax_event_total)
                        except Exception:
                            pass
            except Exception as e:
