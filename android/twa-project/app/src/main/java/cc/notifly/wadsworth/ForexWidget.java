...
public class ForexWidget extends AppWidgetProvider {
    ...
    static final String ACTION_REFRESH = "cc.notifly.wadsworth.FOREX_REFRESH";
        ...
        new Thread(() -> {
            try {
                ...
                if (conn.getResponseCode() != 200) {
                    views.setTextViewText(id(ctx, "widget_forex_subtitle"), "Open app to log in");
                    mgr.updateAppWidget(widgetId, views);
                    return;
                }
                ...
                views.setTextViewText(id(ctx, "widget_forex_subtitle"), "Tap \u21bb to refresh");
                mgr.updateAppWidget(widgetId, views);
            }
        }).start();
    }
