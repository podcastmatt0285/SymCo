...
@router.get("/admin/fix-t6-reward/{player_id}", response_class=HTMLResponse)
def admin_fix_t6_reward(player_id: int, session_token: Optional[str] = Cookie(None)):
    """One-time migration: replace a player's T6 LandPlot reward with a real District."""
    admin = require_admin(session_token)
    if isinstance(admin, RedirectResponse): return admin
    ...
    db = SessionLocal()
    try:
        # Run DDL migration in case is_tutorial_reward column isn't on districts yet
        try:
            from database import run_ddl_migration, engine as _eng
            run_ddl_migration(_eng, "ALTER TABLE districts ADD COLUMN IF NOT EXISTS is_tutorial_reward BOOLEAN DEFAULT FALSE")
            lines.append("✅ DDL migration ran (is_tutorial_reward on districts)")
        except Exception as ex:
            lines.append(f"⚠️ DDL migration: {ex}")

        # Remove old LandPlot tutorial reward(s)
        old_plots = db.query(LandPlot).filter(
            LandPlot.owner_id == player_id,
            LandPlot.terrain_type == "district_food",
            ...
            if plot.occupied_by_business_id:
                ...
                if biz:
                    ...
                    if sale:
                        db.delete(sale)
                    db.delete(biz)
                    lines.append(f"🗑️ Deleted old tutorial business id={biz.id}")
            db.delete(plot)
            lines.append(f"🗑️ Deleted old district_food LandPlot id={plot.id}")

        if not old_plots:
            lines.append("ℹ️ No old district_food LandPlot tutorial reward found.")

        # Check if a tutorial reward District already exists (avoid duplicate)
        existing = db.query(District).filter(
            District.owner_id == player_id,
            District.is_tutorial_reward == True,
        ).first()

        if existing:
            lines.append(f"ℹ️ Tutorial reward District id={existing.id} already exists — skipping creation.")
        else:
            district = District(
                owner_id=player_id,
                district_type="food",
                terrain_type="district_food",
                size=1.0,
                plots_merged=1,
                monthly_tax=0.0,
                ...
                last_tax_payment=datetime.utcnow(),
            )
            db.add(district)
            db.flush()
            ...
            biz = Business(
                ...
                business_type="fast_food_kitchen",
                is_active=True,
                is_tutorial_reward=True,
                created_at=datetime.utcnow(),
            )
            ...
            lines.append(f"✅ Created Food Production Zone District id={district.id}")
            lines.append(f"✅ Created Fast Food Kitchen Business id={biz.id}")
    ...
    body = "<br>".join(lines)
    return f"""<!DOCTYPE html><html><body style="background:#0f172a;color:#e5e7eb;font-family:monospace;padding:32px;">
    <h2 style="color:#38bdf8;">T6 Reward Fix — Player #{player_id}</h2>
    <p>{body}</p>
    <br><a href="/districts" style="color:#38bdf8;">→ Go to Districts</a>
    &nbsp;&nbsp;<a href="/admin/player/{player_id}" style="color:#94a3b8;">→ Admin Player View</a>
    </body></html>"""
