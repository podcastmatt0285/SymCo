...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Collapse to a single column on phones (matches the main dashboard's
       mobile-first sizing). 640px aligns with the app-wide breakpoint so
       tablets/landscape keep multi-column grids instead of stretching
       full-width cards. Inline-grid selectors also catch auto-fit/auto-fill
       rows that would otherwise stay cramped on narrow phones. */
...
async def gas_tracker_page(
    ...
    skin_html = _skin_links(player.id, "crypto") if player else ""
...
.gt-row td {{ padding:12px 14px;font-size:13px; }}
.gt-table th {{ padding:10px 14px;font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #1e293b; }}
...
<div class="container" style="max-width:1100px;">
    ...
    <div class="card" style="padding:0;overflow:hidden;">
        <table class="table gt-table" style="margin:0;">
            <thead><tr>
                <th>Blockchain</th>
                <th style="text-align:right;">Gas Price</th>
                <th style="text-align:center;">Status</th>
                <th style="text-align:right;">Txns/h</th>
...
async def county_governance(
    ...
    <script>
    /* AJAX governance votes — no reload */
    (function() {{
        ...
        document.querySelectorAll('form[action="/api/county/governance/vote"]').forEach(function(form) {{
            form.addEventListener('submit', function(e) {{
                ...
                fetch(form.action, {{method:'POST', body: new URLSearchParams(data)}})
                    ...
                    .then(function(d) {{
                        _toast(d.message || (d.ok ? 'Vote cast' : 'Error'), d.ok);
                        if (d.ok) {{
                            form.querySelectorAll('input[type=number]').forEach(function(i) {{ i.value = ''; }});
                            var pw = form.closest('[id^="weight-preview"]') || document.getElementById('weight-preview-' + (form.querySelector('[id^="burn-"]')||{{}}).id);
